...
public class DAOListExpensesImpl implements DAOListExpenses {
  ...
  public void getConnection() throws ListExpensesException {
    ...
    try {
      ...
      connection = DriverManager.getConnection(dbURL, "admin", "admin");
    } catch (Exception e) {
    ...
    try (Statement statement = connection.createStatement()) {
      ResultSet result =
          statement.executeQuery(
              "select rec.name from listexpenses.receivers as rec where rec.num=" + num + ";");
      if (result.next()) {
  ...
  public Expense getExpense(int num) throws ListExpensesException {
    ...
    try (Statement statement = connection.createStatement()) {
      ResultSet result =
          statement.executeQuery(
              "select exp.paydate,exp.receiver,exp.value from listexpenses.expenses as exp where exp.num="
                  + num
                  + ";");
      if (result.next()) {
  ...
  public int addReceiver(Receiver receiver) throws ListExpensesException {
    ...
    try (Statement statement = connection.createStatement()) {
      String query =
          "insert into listexpenses.receivers (name) values ('" + receiver.getName() + "');";
  ...
  public int addExpense(Expense expense) throws ListExpensesException {
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      String query =
          "insert into listexpenses.expenses (paydate, receiver, value) value ('"
              + date
              + "',"
              + expense.getReseiver()
              + ","
